package com.hidden.artify.ui.camera
...
import com.hidden.artify.databinding.CameraUiContainerBinding
import com.hidden.artify.databinding.FragmentCameraBinding
import com.hidden.artify.extensions.ANIMATION_FAST_MILLIS
import com.hidden.artify.extensions.ANIMATION_SLOW_MILLIS
import com.hidden.artify.extensions.getRealPathFromUri
import com.hidden.artify.extensions.hideKeyboard
import com.hidden.artify.extensions.popBack
import com.hidden.artify.extensions.simulateClick
import com.hidden.artify.ui.edit.EditFragment
import com.hidden.artify.ui.home.HomeViewModel
import com.hidden.artify.ui.main.MainActivity.Companion.KEY_EVENT_ACTION
import com.hidden.artify.ui.main.MainActivity.Companion.KEY_EVENT_EXTRA
import com.hidden.artify.utils.MediaStoreUtils
...
@AndroidEntryPoint
class CameraFragment : BaseFragment<HomeViewModel, FragmentCameraBinding>() {
    ...
    private val displayListener = object : DisplayManager.DisplayListener {
        ...
        override fun onDisplayChanged(displayId: Int) = view?.let { view ->
            if (displayId == this@CameraFragment.displayId) {
                Log.d("salimmm", "Rotation changed: ${view.display.rotation}")
    ...
    override fun onResume() {
        ...
        if (!EditFragment.hasPermissions(requireContext())) {
            popBack()
        } else {
            hideSystemUI()
        }
    ...
    override fun getResourceLayoutId() = R.layout.fragment_camera
...
//    }
    ...
    private fun updateCameraUi() {
        ...
        cameraUiContainerBinding?.root?.let {
            viewBinding.cameraContainer.removeView(it)
        }

        cameraUiContainerBinding = CameraUiContainerBinding.inflate(
            LayoutInflater.from(requireContext()),
            viewBinding.cameraContainer,
            true
        )
                        ...
                        override fun onImageSaved(output: ImageCapture.OutputFileResults) {
                            ...
                            lifecycleScope.launch {
                                if (mediaStoreUtils.getImages().isNotEmpty()) {
                                    val path =
                                        getRealPathFromUri(requireContext(), savedUri)
                                    findNavController().navigate(
                                        CameraFragmentDirections.actionCameraFragmentToEditPhotoFragment(
                                            SizeModel(
                                                icon = path,
                                                size = "256x256",
                                                isSelected = false
...
//                if (mediaStoreUtils.getImages().isNotEmpty()) {
//                    val path =
//                        getRealPathFromUri(requireContext(), mediaStoreUtils.mediaStoreCollection)
//                    findNavController().navigate(
//                        CameraFragmentDirections.actionCameraFragmentToEditFragment(path)
//                    )
//                }
//            }
//        }
